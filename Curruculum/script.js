document.addEventListener('DOMContentLoaded', () => {
    ...
    // (Esta sección funciona correctamente y no tiene cambios)
    ...
    // --- FUNCIÓN RENDERPREVIEW (ACTUALIZADA CON LA SOLUCIÓN DE RUTAS A PRUEBA DE FALLOS) ---
    async function renderPreview(profile) {
        ...
        const templateUrl = selectedTemplate.dataset.templateUrl; // ej: "templates/template-01.html"
        try {
            ...
            // 1. Convertimos el texto HTML en un documento DOM temporal
            const parser = new DOMParser();
            const doc = parser.parseFromString(templateHtml, 'text/html');

            // 2. Buscamos la etiqueta del stylesheet
            const stylesheetLink = doc.querySelector('link[rel="stylesheet"]');
            if (stylesheetLink) {
                const cssPath = stylesheetLink.getAttribute('href'); // ej: "template-01.css"
                // 3. Creamos la URL absoluta y correcta y la reemplazamos
                const absoluteCssUrl = new URL(cssPath, new URL(templateUrl, window.location.href)).href;
                stylesheetLink.setAttribute('href', absoluteCssUrl);
            }
            
            // 4. Inyectamos los datos en este documento DOM corregido
            ...
            const lists = { '#courses-list': profile.courses, '#references-list': profile.references, '#education-list': profile.education, '#experience-list': profile.experience };
            for (const [selector, items] of Object.entries(lists)) {
                const listElement = doc.querySelector(selector);
                if (listElement) {
                    listElement.innerHTML = '';
                    items.forEach(item => {
                        const mainField = item.title || item.name;
                        if (mainField && mainField.trim() !== '') {
                            ...
                            listElement.innerHTML += `<li><span class="item-title">${mainField}</span><span class="item-subtitle">${subtitle}</span></li>`;
                        }
                    });
                }
            }
            
            // 5. Convertimos el documento DOM corregido de nuevo a texto y lo mostramos
