document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- (Secciones 1, 2 y 3 - Selectores, Recorte y DB - no cambian) ---
    let db;
    const DB_NAME = 'perfilesDB', DB_VERSION = 1, STORE_NAME = 'perfiles';
    const A4_WIDTH_PX = 794;
    // ... (resto de selectores y lógica de recorte)
    
    // --- FUNCIÓN RENDERPREVIEW (ACTUALIZADA CON LA SOLUCIÓN MÁS SIMPLE) ---
    async function renderPreview(profile) {
        const selectedTemplate = templateGallery.querySelector('.selected');
        if (!selectedTemplate) { previewArea.innerHTML = '<p>Por favor, selecciona una plantilla.</p>'; return; }
        const templateUrl = selectedTemplate.dataset.templateUrl;
        try {
            const response = await fetch(templateUrl);
            if (!response.ok) throw new Error(`No se pudo cargar la plantilla. Estado: ${response.status}`);
            const templateHtml = await response.text();
            const tempContainer = document.createElement('div');
            tempContainer.innerHTML = templateHtml;
            
            // --- (Lógica de inyección de datos no cambia) ---
            ...
            // ... (resto de la inyección de datos)

            previewArea.innerHTML = '';
            const iframe = document.createElement('iframe');
            iframe.onload = () => scalePreview();
            iframe.style.cssText = 'width: 794px; height: 1123px; transform-origin: top left; border: none;';
            previewArea.appendChild(iframe);
            
            const doc = iframe.contentWindow.document;
            doc.open();
            // --- ¡AQUÍ ESTÁ LA SOLUCIÓN CORREGIDA Y SIMPLIFICADA! ---
            // Simplemente le decimos que la carpeta base es 'templates/'
            doc.write('<base href="templates/">'); 
            doc.write(tempContainer.innerHTML);
            doc.close();

        } catch (error) {
            console.error('Error al renderizar la vista previa:', error);
            previewArea.innerHTML = `<p>Ocurrió un error al cargar la vista previa. Revisa la consola.</p>`;
        }
    }
    
    // --- (El resto del código del script es idéntico a la versión anterior) ---
    ...
    userPhotoInput.addEventListener('change', (e) => {
        ...
        reader.onload = (event) => {
            const img = new Image();
            img.onload = () => {
                if (img.width < 1080 || img.height < 1080) { alert('La imagen es demasiado pequeña. Debe tener al menos 1080px de ancho y alto.'); userPhotoInput.value = ''; return; }
    ...
    cancelCropBtn.addEventListener('click', () => { cropperModal.style.display = 'none'; if (cropper) cropper.destroy(); userPhotoInput.value = ''; });
    cropImageBtn.addEventListener('click', () => {
        if (!cropper) return;
        const canvas = cropper.getCroppedCanvas({ width: 1080, height: 1080, imageSmoothingQuality: 'high' });
        canvas.toBlob((blob) => { croppedImageBlob = blob; cropperModal.style.display = 'none'; if (cropper) cropper.destroy(); }, 'image/jpeg');
    });
    ...
    function saveProfile(profileData) { if (!db) { return; } const transaction = db.transaction([STORE_NAME], 'readwrite'); const store = transaction.objectStore(STORE_NAME); const request = store.add(profileData); request.onsuccess = () => { alert('¡Perfil guardado con éxito!'); userForm.reset(); displayProfiles(); }; request.onerror = (e) => console.error('Error al guardar perfil:', e.target.error); }
    ...
    function setupEventListeners() {
        ...
        addEducationBtn.addEventListener('click', () => addEntry(educationContainer, templates.education)); addExperienceBtn.addEventListener('click', () => addEntry(experienceContainer, templates.experience)); addCourseBtn.addEventListener('click', () => addEntry(coursesContainer, templates.course)); addReferenceBtn.addEventListener('click', () => addEntry(referencesContainer, templates.reference));
        userForm.addEventListener('submit', (event) => { event.preventDefault(); const userProfile = { userName: document.getElementById('user-name').value, cedula: document.getElementById('cedula').value, birthDate: document.getElementById('birth-date').value, nationality: document.getElementById('nationality').value, location: document.getElementById('location').value, phone: document.getElementById('phone').value, email: document.getElementById('email').value, education: [], experience: [], courses: [], references: [], photo: croppedImageBlob || null }; document.querySelectorAll('#education-container .form-entry-group').forEach(group => userProfile.education.push({ title: group.querySelector('.education-title').value, institute: group.querySelector('.education-institute').value, year: group.querySelector('.education-year').value })); document.querySelectorAll('#experience-container .form-entry-group').forEach(group => userProfile.experience.push({ title: group.querySelector('.experience-title').value, company: group.querySelector('.experience-company').value, startYear: group.querySelector('.experience-start-year').value, endYear: group.querySelector('.experience-end-year').value })); document.querySelectorAll('#courses-container .form-entry-group').forEach(group => userProfile.courses.push({ title: group.querySelector('.course-title').value, institute: group.querySelector('.course-institute').value })); document.querySelectorAll('#references-container .form-entry-group').forEach(group => userProfile.references.push({ name: group.querySelector('.reference-name').value, phone: group.querySelector('.reference-phone').value })); saveProfile(userProfile); croppedImageBlob = null; userPhotoInput.value = ''; });
